...
export default function App() {
  ...
  if (!token) return <Identify slug={slug} onAuth={setToken} />;
...
function Activites() {
  ...
  useEffect(() => { post("get-activites", {}).then((r) => setList(r.activites || [])); }, []);
  if (!list) return <Card>Chargement…</Card>;
  if (!list.length) return <Card style={{ color: C.muted }}>Aucune activité publiée pour le moment.</Card>;
  return (
    <>
      {list.map((a, i) => (
